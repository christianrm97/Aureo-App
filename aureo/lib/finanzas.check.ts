...
const base = (): Estado => ({
  ...
  recurrentes: [
    { importe: 1500, tipo: 'ingreso', dia: 28 },
    { importe: -700, tipo: 'gasto', dia: 26 },
    { importe: -250, tipo: 'gasto', dia: 20 },
  ],
...
assert.equal(r.gastosFijos, 950)
assert.equal(round(r.suscripciones), 24.98)
assert.equal(r.recibos, 100)
assert.equal(r.deuda, 120)
assert.equal(round(r.ahorroMensual), round(1500 - 950 - 24.98 - 100 - 120 - 80))
...
assert.equal(rExtra.ingresos, 1500 + 300)
