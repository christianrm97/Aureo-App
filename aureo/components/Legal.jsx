...
export function PaginaLegal({ titulo, subtitulo, children }) {
  const incompleto = [LEGAL.responsable, LEGAL.nif, LEGAL.direccion].some(faltaPorRellenar)
        ...
        {incompleto && (
          <div className="rounded-2xl p-4 mt-5 flex items-start gap-2.5"
            ...
            <div className="text-[12.5px] leading-snug" style={{ color: '#78350F' }}>
              Faltan datos del responsable (titular, NIF o domicilio). Sin ellos este texto no
              cumple el RGPD. Se configuran con las variables <code>NEXT_PUBLIC_LEGAL_*</code>.
            </div>
          </div>
        )}
