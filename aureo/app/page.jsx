...
import {
  ...
  Landmark, PiggyBank, Users, Banknote, LineChart, Wallet, X, ChevronRight,
  TrendingUp, Sparkles, Eye, EyeOff, Repeat, Calendar,
...
const PLAN = {
  nomina: 1410.67, aBleap: 90, aPareja: 150, aMyInvestor: 80,
  irpf: 280.90, prestamoPadres: 686, objetivo: 3663,
}

// `id` viaja a la API y al Atajo de iPhone (ASCII, estable).
// `label` es lo unico que se pinta.
const CATEGORIAS = [
...
function calcularProyeccion(saldoInicialLiquido, meses = 19) {
  const hoy = new Date()
  const inicio = new Date(hoy.getFullYear(), hoy.getMonth(), 1)
  const data = []
  let liquido = saldoInicialLiquido
  for (let i = 0; i < meses; i++) {
    const d = new Date(inicio.getFullYear(), inicio.getMonth() + i, 1)
    const mesLabel = d.toLocaleDateString('es-ES', { month: 'short' }).replace('.', '')
    const yr = String(d.getFullYear()).slice(2)
    const esSeptOn = (d.getFullYear() > 2025) || (d.getFullYear() === 2025 && d.getMonth() >= 8)
    const neto = PLAN.nomina - PLAN.prestamoPadres - PLAN.aBleap - PLAN.aPareja - (esSeptOn ? PLAN.irpf : 0) - (esSeptOn ? PLAN.aMyInvestor : 0)
    liquido = Math.max(0, liquido + neto)
    data.push({ mes: `${mesLabel} ${yr}`, liquido: Math.round(liquido), objetivo: PLAN.objetivo })
  }
  return data
}

const fmt = (n) => new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(n)
const fmt2 = (n) => new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n)
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [g, r, p] = await Promise.all([
        api('gastos?limit=100').catch(() => ({ items: [] })),
        api('recurrentes').catch(() => ({ items: [] })),
        api('precios').catch(() => ({ data: null })),
      ])
      setGastos(g.items || [])
      setRecurrentes(r.items || [])
      setPrecios(p.data || null)
    })()
  }, [])
  ...
  const gastadoTotal = useMemo(() => gastos.reduce((s, g) => s + g.importe, 0), [gastos])

  // Precio en vivo sobre MyInvestor: % de cambio aplicado al saldo base
  const cuentas = useMemo(() => {
    const spChange = precios?.sp500?.changePct ?? 0
    return CUENTAS_BASE.map(c => {
      if (c.id === 'myinvestor' && precios?.sp500?.price) {
        const ajuste = c.saldo * (spChange / 100)
        return { ...c, saldo: c.saldo + ajuste, changePct: spChange, live: true }
      }
      return c
    })
  }, [precios])
  ...
  const progreso = Math.min(100, Math.max(0, (liquido / PLAN.objetivo) * 100))
  const proyeccion = useMemo(() => calcularProyeccion(liquido), [liquido])
  ...
  return (
    <div className="min-h-screen pb-28" style={{background:'var(--aureo-bg)'}}>
      <div className="max-w-md mx-auto px-4 pt-3">
        ...
        <div className="flex items-center justify-between py-3">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-full grid place-items-center text-white font-semibold text-sm" style={{background:'var(--aureo-purple)'}}>C</div>
            <div>
              <div className="text-[11px] uppercase tracking-widest text-[color:var(--aureo-text-mute)]">Aureo</div>
              <div className="text-[14px] font-semibold -mt-0.5">Hola, Christian</div>
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            <button onClick={()=>setOculto(v=>!v)} aria-label={oculto ? 'Mostrar importes' : 'Ocultar importes'} className="w-9 h-9 rounded-full grid place-items-center" style={{background:'#fff', border:'1px solid var(--aureo-border)'}}>
              {oculto ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            </button>
            <button aria-label="Notificaciones" className="w-9 h-9 rounded-full grid place-items-center relative" style={{background:'#fff', border:'1px solid var(--aureo-border)'}}>
              <Bell className="w-4 h-4" />
              <span className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full" style={{background:'var(--aureo-purple)'}}></span>
            </button>
          </div>
        </div>
        ...
        {tab === 'home' && (
          <>
            ...
            <ActionsRow onAdd={() => setModalOpen(true)} onAnalysis={() => setTab('analysis')} onGoal={() => setTab('goal')} onMove={() => setTab('recurring')} />
            <SpaceObjetivo liquido={liquido} objetivo={PLAN.objetivo} progreso={progreso} oculto={oculto} />
            <SectionHeader title="Cuentas" />
            <CuentasLista cuentas={cuentas} gastadoTotal={gastadoTotal} oculto={oculto} />
            <SectionHeader title="Próximas" right="Ver todas" onRight={() => setTab('recurring')} />
            <RecurrentesWidget recurrentes={recurrentes} oculto={oculto} />
            <SectionHeader title="Proyección" right="Ene 2027" />
            <ProyeccionCard data={proyeccion} objetivo={PLAN.objetivo} />
        ...
        {tab === 'analysis' && (
          <AnalisisView gastos={gastos} onBack={() => setTab('home')} oculto={oculto} />
        )}

        {tab === 'recurring' && (
          <RecurrentesView recurrentes={recurrentes} onBack={() => setTab('home')} oculto={oculto} />
        )}

        {tab === 'goal' && (
          <GoalView liquido={liquido} objetivo={PLAN.objetivo} progreso={progreso} proyeccion={proyeccion} onBack={() => setTab('home')} oculto={oculto} />
        )}
      </div>
      ...
      <AnimatePresence>
        {modalOpen && <ModalGasto onClose={() => setModalOpen(false)} onSubmit={handleAddGasto} />}
      </AnimatePresence>
    </div>
  )
}
...
// -------------- HERO --------------
function HeroCard({ patrimonio, liquido, oculto, spChange }) {
...
function ActionsRow({ onAdd, onAnalysis, onGoal, onMove }) {
  const items = [
    { label: 'Añadir',   icon: Plus,    onClick: onAdd, primary: true },
    { label: 'Recurr.',  icon: Repeat,  onClick: onMove },
    { label: 'Objetivo', icon: Target,  onClick: onGoal },
    { label: 'Análisis', icon: PieIcon, onClick: onAnalysis },
  ]
  return (
...
function SectionHeader({ title, right, onRight }) {
  return (
...
function SpaceObjetivo({ liquido, objetivo, progreso, oculto }) {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="aureo-card mt-5 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[12px]" style={{color:'var(--aureo-text-dim)'}}>Espacio de ahorro</div>
          </div>
        </div>
      ...
      <div className="h-2 rounded-full overflow-hidden" style={{background:'#F0EBF6'}}>
        <motion.div initial={{ width: 0 }} animate={{ width: `${progreso}%` }} transition={{ duration: 1, ease: [0.22,1,0.36,1] }} className="h-full rounded-full" style={{background:'linear-gradient(90deg, #6C2BD9 0%, #8B5CF6 100%)'}} />
      </div>
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="aureo-card overflow-hidden">
      {cuentas.map((c, i) => {
        ...
        const isBleap = c.id === 'bleap'
        const saldo = isBleap ? Math.max(0, c.saldo - gastadoTotal) : c.saldo
        return (
          <div key={c.id} className="flex items-center gap-3 px-5 py-4" style={{borderTop: i === 0 ? 'none' : '1px solid var(--aureo-border)'}}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {c.live && <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded font-semibold flex items-center gap-0.5" style={{background:'#DCFCE7', color:'#15803D'}}><span className="w-1.5 h-1.5 rounded-full" style={{background:'#22C55E'}}></span>LIVE</span>}
              </div>
...
function RecurrentesWidget({ recurrentes, oculto }) {
  ...
  const proximos = [...recurrentes]
    .map(r => ({ ...r, diasHasta: ((r.dia - today) + 30) % 30 }))
...
// -------------- PROYECCION --------------
function ProyeccionCard({ data, objetivo }) {
    ...
    <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="aureo-card p-5">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          {alcanza ? (
            <div className="text-[15px] font-semibold mt-0.5">Objetivo en <span style={{color:'var(--aureo-purple)'}}>{alcanza.mes}</span></div>
          ) : (
            <div className="text-[15px] font-semibold mt-0.5">Camino al objetivo</div>
          )}
        </div>
        <div className="flex items-center gap-1 text-[12px] font-medium" style={{color:'var(--aureo-green)'}}>
          <Sparkles className="w-3.5 h-3.5" /> En camino
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        <MiniStat label="Nómina" value={fmt(PLAN.nomina)} tone="positive" />
        <MiniStat label="Préstamo" value={fmt(PLAN.prestamoPadres)} tone="negative" />
        <MiniStat label="IRPF" value={fmt(PLAN.irpf)} tone="negative" hint="desde sept" />
      </div>
    </motion.section>
  )
}
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="aureo-card p-8 text-center">
        <div className="w-14 h-14 rounded-full grid place-items-center mx-auto mb-3" style={{background:'var(--aureo-purple-soft)'}}>
          <ArrowUpRight className="w-6 h-6" style={{color:'var(--aureo-purple)'}} />
        </div>
        <div className="text-[14px] font-semibold">Sin gastos aún</div>
        <div className="text-[12px] mt-1" style={{color:'var(--aureo-text-dim)'}}>Pulsa <b>+</b> para añadir uno rápidamente</div>
      </motion.div>
    )
  }
  return (
    <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="aureo-card overflow-hidden">
      <AnimatePresence>
        {gastos.slice(0, 8).map((g, i) => {
          ...
          const isShortcut = g.source === 'shortcut'
          return (
            <motion.div key={g.id}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[15px] font-semibold truncate flex items-center gap-1.5">
                  {g.nota || cat.label}
                  {isShortcut && <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded font-semibold" style={{background:'#DBEAFE', color:'#1D4ED8'}}>iPhone</span>}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => onDelete(g.id)} aria-label="Borrar gasto" className="opacity-0 group-hover:opacity-100 focus-visible:opacity-100 transition w-7 h-7 rounded-full grid place-items-center" style={{background:'var(--aureo-bg)'}}>
...
// -------------- RECURRENTES VIEW --------------
function RecurrentesView({ recurrentes, onBack, oculto }) {
  const today = new Date().getDate()
  const items = [...recurrentes].map(r => ({ ...r, diasHasta: ((r.dia - today) + 30) % 30 })).sort((a,b) => a.diasHasta - b.diasHasta)
  const totalMensual = recurrentes.filter(r=>r.tipo!=='ingreso').reduce((s,r)=>s+r.importe,0)
  const totalIngresos = recurrentes.filter(r=>r.tipo==='ingreso').reduce((s,r)=>s+r.importe,0)

  return (
    <>
      <PageHeader title="Recurrentes" onBack={onBack} />
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="aureo-card p-4">
          <div className="text-[11px] uppercase tracking-wider" style={{color:'var(--aureo-text-mute)'}}>Ingresos</div>
          <div className="tabular text-[20px] font-semibold mt-1" style={{color:'#22C55E'}}>+{oculto ? '•••' : fmt(totalIngresos)}</div>
        </div>
        <div className="aureo-card p-4">
          <div className="text-[11px] uppercase tracking-wider" style={{color:'var(--aureo-text-mute)'}}>Gastos fijos</div>
          <div className="tabular text-[20px] font-semibold mt-1" style={{color:'#EF4444'}}>{oculto ? '•••' : fmt(totalMensual)}</div>
        </div>
      </div>

      <div className="aureo-card overflow-hidden">
        {items.map((r, i) => {
          const Icon = ICONS[r.icono] || Sparkles
          const cat = catDe(r.categoria)
          const isIngreso = r.tipo === 'ingreso'
          return (
            <div key={r.id} className="flex items-center gap-3 px-5 py-4" style={{borderTop: i === 0 ? 'none' : '1px solid var(--aureo-border)'}}>
              <div className="w-11 h-11 rounded-full grid place-items-center flex-shrink-0" style={{background: isIngreso ? '#DCFCE7' : cat.bg}}>
                <Icon className="w-5 h-5" style={{color: isIngreso ? '#22C55E' : cat.color}} strokeWidth={2.1} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[14px] font-semibold">{r.nombre}</div>
                <div className="text-[12px] flex items-center gap-1" style={{color:'var(--aureo-text-dim)'}}>
                  <Calendar className="w-3 h-3" /> Día {r.dia} · {r.diasHasta === 0 ? 'hoy' : `en ${r.diasHasta} días`}
                  {r.desde && <span> · desde {r.desde}</span>}
                </div>
              </div>
              <div className="tabular text-[14px] font-semibold" style={{color: isIngreso ? '#22C55E' : '#14101B'}}>
                {oculto ? '•••' : `${isIngreso ? '+' : ''}${fmt2(r.importe)}`}
...
// -------------- GOAL VIEW --------------
function GoalView({ liquido, objetivo, progreso, proyeccion, onBack, oculto }) {
  const alcanza = proyeccion.find(d => d.liquido >= objetivo)
  return (
    <>
      ...
      <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="hero-gradient rounded-[28px] p-6 text-white">
        ...
        <div className="text-[13px] text-white/80 mt-1">Actualmente: <span className="font-medium text-white tabular">{oculto ? '•••' : fmt(liquido)}</span></div>
        ...
        <div className="flex justify-between mt-2 text-[12px] text-white/80">
          ...
          {alcanza && <span>Se alcanzará en {alcanza.mes}</span>}
        </div>
      </motion.section>
      <div className="mt-4">
        <ProyeccionCard data={proyeccion} objetivo={objetivo} />
      </div>
    </>
  )
}
...
function PageHeader({ title, onBack }) {
  return (
    <div className="flex items-center gap-3 py-3 mb-2">
      <button onClick={onBack} aria-label="Volver" className="w-9 h-9 rounded-full grid place-items-center" style={{background:'#fff', border:'1px solid var(--aureo-border)'}}>
...
function BottomNav({ onAdd, tab, setTab }) {
  const items = [
    ...
    { id: 'analysis',  label: 'Análisis', icon: PieIcon },
    { id: 'add',       label: '',         icon: Plus, primary: true, onClick: onAdd },
    { id: 'recurring', label: 'Recurr.',  icon: Repeat },
...
// -------------- MODAL GASTO --------------
function ModalGasto({ onClose, onSubmit }) {
  ...
  const submit = async (e) => {
    ...
    setError(null)
    try {
      await onSubmit({ nota: nota.trim(), importe: val, categoria })
    } catch {
