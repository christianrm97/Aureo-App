...
import VigilanteView from '@/components/VigilanteView'
  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: 'var(--aureo-bg)' }}>
      <div className="max-w-md mx-auto px-4 pt-3">
        ...
        {tab === 'vigilante' && <VigilanteView onBack={() => setTab('home')} oculto={oculto} />}
...
/** Acceso al Vigilante de gastos desde la home. */
function VigilanteAcceso({ onAbrir }) {
  return (
    <motion.button initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} onClick={onAbrir}
      className="aureo-card w-full mt-3 p-5 flex items-center gap-3 text-left">
      <div className="w-11 h-11 rounded-full grid place-items-center flex-shrink-0" style={{ background: '#CCFBF1' }}>
        <Eye className="w-5 h-5" style={{ color: '#0F766E' }} strokeWidth={2.1} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-[14px] font-semibold">Vigilante de gastos</div>
        <div className="text-[12px]" style={{ color: 'var(--aureo-text-dim)' }}>
          Sube el CSV de tu banco y caza suscripciones olvidadas
        </div>
      </div>
    </motion.button>
  )
